...
int main (void){
	...
	printf("PRUEBA 1: Cadena SIN salto de línea\n");
	testQuitaSaltoLinea(c1);

	printf("PRUEBA 2: Cadena CON salto de línea\n");
	testQuitaSaltoLinea(c2);
	return 0;
}

void testQuitaSaltoLinea(Cadena c){
	printf("Cadena ANTES de quitar salto de línea: ");
	mostrarCaracteresCadena(c);

	quitaSaltoDeLinea(c);

	printf("Cadena DESPUES de quitar salto de línea: ");
	mostrarCaracteresCadena(c);
}
