...
public class DepartamentoImpl implements Departamento {
	...
	public Profesor getProfesorMaximaDedicacionMediaPorAsignatura(){
		Comparator<Profesor> cmp = Comparator.comparing(p -> p.getDedicacionTotal()/p.getAsignaturas().size());
		return getProfesores().stream().filter(p -> !p.getAsignaturas().isEmpty()).max(cmp).get();
	}
